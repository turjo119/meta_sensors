...
playback=False
playbackFile="Testdata/Test6_13.3.csv"
f=None;

def gauss(x,mean,sig):
    return 1/(np.sqrt(2*np.pi)*sig)*np.exp(-0.5*((x-mean)/sig)**2) 

kernal=np.zeros(20,dtype=np.double)
bufferl=len(kernal)
for i in range(bufferl):
    kernal[i]=gauss(i,bufferl/2,bufferl/4)
...
if(playback):
    ...
    ser = serial.Serial('/dev/ttyACM0',960,timeout=0.01)  # open serial port
...
try:
    ...
    (l00,l01,l10,l11,l20,l21,p0,p1)=0,0,0,0,0,0,0,0
at=getAngle(l00,l01,l10,l11,l20,l21,p0,p1)
buffer=np.zeros(bufferl,dtype=np.double)
while True:
    ...
    if(time.time()-lastTime>0.003):
        while(time.time()-lastTime>0):
            ...
            buffer=np.append(buffer[1:],at)
        
    screen.fill(black)

    a=np.sum(kernal*buffer)
    print(a*180//math.pi)
    a=a+math.pi
